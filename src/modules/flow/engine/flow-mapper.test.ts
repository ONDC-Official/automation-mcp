...
/**
 * Ported verbatim from the workbench's mock-playground service
 * (`src/service/flows/flow-mapper.test.ts`), because the mapper was ported
 * verbatim too. It is the calibration suite for the whole loop: every status
 * the model ever sees is decided here, and these cases encode behaviour that
 * was settled against a real network rather than reasoned out.
 *
 * Changes from upstream are mechanical only — vitest imports, our type names,
 * and `Flow`/`TransactionCache` becoming `EngineFlow`/`EngineTransaction`.
 * Anything else diverging is a defect, not an improvement.
 */

// ---------- builders ----------

const makeStep = (
  overrides: Partial<EngineSequenceStep> & {
    key: string;
    type: string;
    owner: "BAP" | "BPP";
  },
...
const makeTxn = (
  ...
  // Shaped like the real transaction record, which carries more than the
  // engine ever reads. Assigned through a variable so the extra fields are
  // allowed, exactly as they are at the real call site.
  const record = {
    flowId: "flow1",
    latestAction: "search",
...
const emptySession: EngineSessionData = {};
...
describe("flow-mapper", () => {
  ...
  describe("getFlowCompleteStatus — API sequencing (mismatches)", () => {
    ...
    it("unknown API (not in flow) → missed step with index -1", () => {
      ...
      const result = getFlowCompleteStatus(
        makeTxn(
          [apiHistory("on_cancel", "m1", "2024-01-01T00:00:00.000Z")],
          "BAP",
        ),
        flow,
        "AVAILABLE",
  ...
  describe("getFlowCompleteStatus — FORM sequencing", () => {
    it("form match at expected step advances pointer", () => {
      ...
      const result = getFlowCompleteStatus(
        makeTxn(
          [
            formHistory("DYNAMIC_FORM", "form1", "2024-01-01T00:00:00.000Z", {
              submissionId: "sub1",
            }),
          ],
          "BAP",
    ...
    it("form out of order (type exists later) → missed step", () => {
      ...
      const result = getFlowCompleteStatus(
        makeTxn(
          [formHistory("HTML_FORM", "form1", "2024-01-01T00:00:00.000Z")],
          "BAP",
        ),
        flow,
        "AVAILABLE",
    ...
    it("form not in flow → missed step with index -1", () => {
      ...
      const result = getFlowCompleteStatus(
        makeTxn(
          [formHistory("HTML_FORM", "ghost-form", "2024-01-01T00:00:00.000Z")],
          "BAP",
        ),
        flow,
        "AVAILABLE",
      ...
      const result = getFlowCompleteStatus(
        makeTxn(
          [
            ...
            formHistory("DYNAMIC_FORM", "form2", "2024-01-01T00:00:01.000Z", {
              submissionId: "sub2",
            }),
          ],
          "BAP",
  ...
  describe("getFlowCompleteStatus — MORE_SEQUENCE extension", () => {
    it("appends MORE_SEQUENCE steps to flow sequence", () => {
      ...
      const session: EngineSessionData = {
        MORE_SEQUENCE: [
          makeStep({ key: "k2", type: "on_search", owner: "BPP" }),
        ],
      };
    ...
    it("dedupes identical action|messageId into a single matched step but accumulates payloads", () => {
      ...
      const result = getFlowCompleteStatus(
        makeTxn(
          [
            {
              ...apiHistory("on_search", "m1", "2024-01-01T00:00:00.000Z", {
                ack: "NACK",
                payloadId: "p-nack",
              }),
            },
            {
              ...apiHistory("on_search", "m1", "2024-01-01T00:00:01.000Z", {
                ack: "ACK",
                payloadId: "p-ack",
              }),
            },
      ...
      const result = getFlowCompleteStatus(
        makeTxn(
          [
            ...
            formHistory("DYNAMIC_FORM", "form1", "2024-01-01T00:00:01.000Z", {
              submissionId: "sub1",
            }),
          ],
          "BAP",
  ...
  describe("extras — pair resolution", () => {
    ...
    it("E4: A fires, then B (pair) with same messageId → both COMPLETE, no orphans", () => {
      const result = getFlowCompleteStatus(
        makeTxn(
          [
            apiHistory("on_update", "m1", "2024-01-01T00:00:00.000Z"),
            apiHistory("update", "m1", "2024-01-01T00:00:01.000Z"),
          ],
          "BAP",
        ),
        buildFlow(),
    ...
    it("E5: B fires first, then A → bidirectional resolution works", () => {
      const result = getFlowCompleteStatus(
        makeTxn(
          [
            apiHistory("update", "m1", "2024-01-01T00:00:00.000Z"),
            apiHistory("on_update", "m1", "2024-01-01T00:00:01.000Z"),
          ],
          "BAP",
        ),
        buildFlow(),
    ...
    it("E6: same extras key fires twice with different messageIds → two independent sub-flows", () => {
      const result = getFlowCompleteStatus(
        makeTxn(
          [
            apiHistory("on_update", "m1", "2024-01-01T00:00:00.000Z"),
            apiHistory("on_update", "m2", "2024-01-01T00:00:01.000Z"),
          ],
          "BAP",
        ),
        buildFlow(),
      ...
      const awaitingIds = placeholders.map((p) => p.awaitingMessageId).sort();
  ...
  describe("extras — interactions with strict sequence", () => {
    ...
    it("E15: MORE_SEQUENCE + extras coexist independently", () => {
      ...
      const result = getFlowCompleteStatus(
        makeTxn(
          [
            apiHistory("search", "m1", "2024-01-01T00:00:00.000Z"),
            apiHistory("on_search", "m2", "2024-01-01T00:00:01.000Z"),
            apiHistory("on_update", "m3", "2024-01-01T00:00:02.000Z"),
          ],
          "BAP",
        ),
        flow,
      ...
      expect(result.sequence.every((s) => s.status === "COMPLETE")).toBe(true);
  ...
  describe("extras — pair edge cases", () => {
    ...
    it("E16: extras-status WORKING → placeholder uses RESPONDING (not INPUT-REQUIRED) for input-bearing pair", () => {
      ...
      const extraStatuses = new Map<string, FlowStatusCode>([
  ...
  describe("manual flag — gates auto-RESPONDING into INPUT-REQUIRED", () => {
    it("manual + subscriber != owner + no input + AVAILABLE → INPUT-REQUIRED", () => {
      ...
      const input = result.sequence[0]!.input as Array<Record<string, unknown>>;
